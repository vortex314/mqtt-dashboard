...
*/
class MyMqtt {
    constructor(host, port, eventbus, logger) {
        this.eb = eventbus;
        this.log = logger;
    ...
    onConnect() {
        this.connected = true;
        this.eb.emitLocal("mqtt/connected");
        this.log('Connected to ' + this.host + ':' + this.port + " " + this.path); // Connection succeeded; subscribe to our topic
        this.log("Subscribe to topic : " + this.topicFilter);
        var i;
    ...
    connect() {
        ...
        var options = {
            ...
            onFailure: function (message) {
                this.log("Connection failed: " + message.errorMessage + "Retrying");
        ...
        this.log("Connecting Host=" + this.host + ", port=" + this.port + ", path=" + this.path + " as source " + this.name + " ...");
        ...
        try {
            ...
            this.log(' mqtt connect failed ' + error);
        }
    ...
    onConnectionLost(response) {
        this.eb.emitLocal("mqtt/disconnected");
        this.connected = false;
        if (this.autoReconnect)
            setTimeout(() => this.connect(), this.reconnect_timeout);
        this.log("connection lost: " + response.errorMessage + ". Reconnecting :" + this.autoReconnect);
    }
    //=======================================================================
    publish(topic, data) {
    ...
    subscribe(topicPattern) {
        this.log("Subscribe to topic : " + topicPattern);
    ...
    onMessageArrived(message) {
        ...
        this.eb.emit(topic, { "topic": message.destinationName, "message": message.payloadString });
        //        this.eb.emitLocal("mqtt/publish", { "topic": message.destinationName, "message": message.payloadString });
    }
