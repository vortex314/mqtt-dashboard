...
class MyMqtt {
    ...
    onConnect() {
        ...
        this.mqtt.subscribe(this.topicFilter, { qos: 0 });
    ...
    unsubscribe(topicPattern) {
        log("Subscribe to topic : " + topicPattern);
        this.mqtt.unsubscribe(topicPattern, { qos: 0 });
    }
    ...
    onMessageArrived(message) {
        ...
        /*        var row = {};
                row.topic = topic;
                row.message = message.payloadString;
                row.count = 1;
                row.date = new Date();
                var w = observableArray.toArray().find(x => x.topic === topic);
                if (w === undefined) {
        ...
        /* try {
             ...
             log(" invalid mqtt message, JSON parsing failed on : " + topic + '=' + message.payloadString)
             return;
         }*/
